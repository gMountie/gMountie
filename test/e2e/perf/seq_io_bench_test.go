...
func benchmarkSeqReadOpt(b *testing.B, size int) {
	env := setupBenchEnvOpt(b)
	seedPath := filepath.Join(env.dataDir, "seq_read.bin")
	seedFile(b, seedPath, size)
	env.AssertReady(b)
...
func BenchmarkSeqReadOpt1MiB(b *testing.B)  { benchmarkSeqReadOpt(b, sizeMiB1) }
func BenchmarkSeqReadOpt16MiB(b *testing.B) { benchmarkSeqReadOpt(b, sizeMiB16) }
func BenchmarkSeqReadOpt64MiB(b *testing.B) { benchmarkSeqReadOpt(b, sizeMiB64) }

func benchmarkSeqWriteOpt(b *testing.B, size int) {
	env := setupBenchEnvOpt(b)
	clientPath := filepath.Join(env.mountPoint, "seq_write.bin")
...
func BenchmarkSeqWriteOpt1MiB(b *testing.B)  { benchmarkSeqWriteOpt(b, sizeMiB1) }
func BenchmarkSeqWriteOpt16MiB(b *testing.B) { benchmarkSeqWriteOpt(b, sizeMiB16) }
func BenchmarkSeqWriteOpt64MiB(b *testing.B) { benchmarkSeqWriteOpt(b, sizeMiB64) }
