...
import (
	...
	clientiotransport "go.gmountie.dev/gmountie/pkg/client/io/transport"
...
func (s *ResilienceSuite) TestResumesWithinGrace() {
	...
	backend := clientiotransport.NewBackendClient(appCtx.GetClient(), appCtx.GetVolumes()[0].Name)
...
func (s *ResilienceSuite) TestFdOpFailsCleanlyPastGrace() {
	...
	backend := clientiotransport.NewBackendClient(appCtx.GetClient(), "vol")
