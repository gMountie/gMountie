...
description: How gMountie talks. Five gRPC services over HTTP/2, with optional snappy compression and every request scoped to a named volume.
---

# The wire protocol

**Each FUSE syscall on the client becomes one (or a small number of) gRPC calls to the server, framed in HTTP/2.** Five services split the work; every request carries the volume name so the server picks the right backing directory. Compression is opt-in (`rpc.compression: snappy`, default `none`).
...
| ----------- | ------------------------------------------------------------ | ------------------------------------------------ |
| **fs**      | Metadata — directory and inode-level ops, plus push invalidations. | `GetAttr`, `SetAttr`, `ReadDir`, `Mkdir`, `Rename`, `Unlink`, `Subscribe` (push invalidations) |
...
- **Snappy compression (opt-in)** — a custom codec registered on both sides, enabled with `rpc.compression: snappy` (default `none`). It pays off on slow WAN links; on fast links the compressor itself becomes the bottleneck, which is why it's off by default — see [Performance § 2.7](../design/performance.md). Codec lives at `pkg/server/grpc/snappy`.
...
Where the protocol can, RPCs are designed to be **idempotent**: the same `Read(volume, fd, offset, length)` returns the same bytes; `Write` carries the offset rather than relying on an implicit cursor. The client retries transient failures (`Unavailable`, `DeadlineExceeded`) on operations whose retry is safe, inside a wall-clock window (`rpc.retry_window`, default 60 s) with fresh per-attempt deadlines — see the implementation in `pkg/client/backend/transport/retry.go`.
