...
- [ ] **Test** `ReconnectOpenFDSuite`:
  - `TestOpenFDSurvivesConnectionDrop`: write a file server-side; mount; `os.Open` it and read the first chunk (holds a server-side fd). `proxy.Sever()` → the gRPC conn drops, keepalive errors, the client enters recovery. Wait briefly (**well under GracePeriod**, e.g. 1–2s vs the 30s default, so the reap timer never fires). `proxy.Restore()`. Assert the **strong contract**: the session recovers via **Resume** with the **same `SessionID()`** (capture before/after via `GetClient().SessionID()` — same id proves reclaim, not a fresh Create), and **continue reading the same open fd to EOF with correct content**. A failed/ESTALE read here is a **real bug**, not an accepted outcome — fail the test.
  - `TestReadAfterRestoreContentMatches`: end-to-end sha256 of the bytes read across the drop equals the written content.
- [ ] **Contract (confirmed against `session.go`):** `MarkDisconnected` only *schedules* `sess.ReleaseAll()` after `GracePeriod`; it does not close fds on disconnect. `Resume` does `reapers.LoadAndDelete(id) → cancel()`, cancelling the reap before `ReleaseAll` runs — so a drop+Resume within the grace period leaves the session **and all open fds intact**, and Resume reattaches the **same session_id** so the client's in-flight fd references still match. The test asserts this strong "fd survives" behavior. (Only a true reap — grace elapsed — or `Stop()` releases fds; that's the separate server-killed case in T3, where fds are legitimately gone and the mount self-heals via fresh Create.)
