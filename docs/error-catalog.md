...
| `invalid auth type: "X" (only 'basic' is supported)`              | `pkg/client/config`   | The client's `auth.type` isn't `basic`. (The client selects mTLS via its TLS client-cert settings, not this field.) | Set `auth.type: basic` in the client config. The server accepts `basic` or `mtls` (server-side string: `invalid auth type: "X" (supported: basic, mtls)`). |
