...
| `invalid auth type: "X" (only 'basic' and 'mtls' are supported)`  | `pkg/client/config`   | The client's `auth.type` isn't `basic` or `mtls`.                              | Set `auth.type: basic` (username/password) or `auth.type: mtls` (client cert under `server.tls`). The server-side equivalent string is `invalid auth type: "X" (supported: basic, mtls)`. |
| `invalid mount type: <X>`                                         | `pkg/client/config`   | `mount.type` isn't `single`.                                                   | Set `mount.type: single`. See [Client config](./client/config.md#mount-configuration). |
...
| `config is empty`                                                 | `pkg/client/grpc`     | The client tried to build a gRPC connection from a nil/empty config.          | Supply a config: `-c client.yaml`, `--profile <name>`, the CLI flags, or `--credentials`. See [Client config](./client/config.md). |
...
| Error                                          | Component            | What it means                                                                                  | What to check                                                                                                                                       |
| ---------------------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mount fail: <reason>`                         | `pkg/client/mount`   | FUSE attach (the kernel-side mount step) failed.                                               | Inspect `<reason>` — usually `fusermount3` is missing, the mountpoint is not empty, or the user lacks `/dev/fuse` access. On macOS a missing-FUSE-provider error is annotated with a macFUSE/FUSE-T install hint. |
| `regular unmount: <reason>; lazy unmount fallback: <reason>` | `pkg/client/mount` | FUSE detach failed on shutdown — both the regular unmount retries and the `fusermount3 -uz` lazy fallback. | Process holding files open in the mount? Use `lsof +D /mnt/...` to find it, kill, then `umount -l /mnt/...` as a last resort. |
| `open cache persist: <reason>`                 | `pkg/client/mount`   | The per-volume disk cache could not be opened — most commonly its `LOCK` file is held by another client process on the same cache dir. | One client process per cache dir: stop the other `gmountie mount`, or give this mount its own `cache.path` (profiles each get their own). |
...
| `No such file or directory`        | `ENOENT` / `NotFound`        | The server's volume directory doesn't contain that path.                                                                   | Confirm the path exists on the server's filesystem inside the volume's `path`. The negative-cache TTL (`cache.negative_ttl`, default 30 s) may briefly remember a 404 — wait it out or restart the mount.                       |
...
| `server sent more bytes than requested` | `pkg/client/backend/transport` | A streaming Read returned more bytes than the client asked for — a protocol bug. Mount will recover but the read is aborted. | If reproducible, [open an issue](https://github.com/gMountie/gMountie/issues) with a verbose-log excerpt — this shouldn't happen.                                                                                            |
