...
- **Open files survive a server restart.** When a `gmountie serve` process restarts, open file descriptors no longer break: the client transparently reopens each handle against the new session and the application's reads/writes resume without an error — no external session database. To stay safe for multi-client volumes the server now mints a per-process **boot epoch** (`boot_epoch` on `SessionCreateReply`, an additive field): the client reclaims a handle only when the epoch changes (a true restart, where no other client could have mutated the file while the server was down) and fails cleanly when the same process merely reaped an idle session past its grace period. Reopen flags are sanitized (`O_CREAT`/`O_EXCL`/`O_TRUNC` stripped) so a reclaim never truncates the file being written, and the handle reopens under the original opener's identity. Byte-range locks are not re-asserted across a restart, and an unlinked-but-open file cannot be recovered (its path is gone). See [Reliability & Recovery](design/reliability-and-recovery.md).

### Fixes

- **SQLite WAL mode no longer `SIGBUS`es (and bricks) the database over a mount.** Opening a SQLite database in WAL mode used to crash with `Bus error` — SQLite `MAP_SHARED`-mmaps the `-shm` sidecar, which the kernel can't back over FUSE — and the leftover `-wal`/`-shm` pinned the DB in WAL so every subsequent open crashed too, leaving it unopenable until the sidecars were deleted on another filesystem. The client now opens `*-shm` sidecars with `FOPEN_DIRECT_IO`, so the kernel refuses that mmap with a clean error: SQLite returns a normal I/O error instead of bus-faulting, and the DB stays openable (data was never corrupted). A new `fuse.direct_io` knob (default off) forces direct-IO on every handle for general `MAP_SHARED` mmap workloads (LMDB, SQLite `mmap_size>0`). For a working journal on the mount use `PRAGMA journal_mode=DELETE`.
- **`gmountie status` shows session health (no more zombie "active" mounts).** When a mount's session could no longer authenticate (revoked/expired cert), every FS op returned `Permission denied` while `status` still reported the mount `active`. The running mount now heartbeats its session-liveness into its on-disk record, and `status` adds a `STATUS` column: `active`, `degraded` (process alive but session locked out / reconnecting), or `stale` (heartbeat stopped — daemon wedged). Old records with no heartbeat read as `active`.
- **`gmountie unmount` is now idempotent and honest about busy mounts.** A busy unmount (open files under the mount) used to wait 10 s and then fail with `rc=1` even though the lazy detach succeeded, and a second `unmount` of an already-detached path failed with `rc=1` (`not mounted`). Unmount now succeeds (`rc=0`) as soon as the mountpoint detaches — reporting that a background process is still draining open files — and treats an already-unmounted path as success. Only a mount still attached after the timeout is an error.
- **`gmountie mount --daemon` reports the real failure reason.** A background mount that failed on a local condition (e.g. the cache lock when the volume is already mounted) surfaced only as a generic `daemon child exited before signalling mount ready` timeout, with the real cause buried in `mount-daemon.log`. The child now sends its actual error up the ready pipe so the parent prints it directly; re-mounting an already-mounted path also fails fast with a clear message before any certificate is minted.
- **`GMOUNTIE_FUSE_*` environment variables were silently ignored.** `GMOUNTIE_FUSE_ATTR_TIMEOUT_SECS`, `GMOUNTIE_FUSE_ENTRY_TIMEOUT_SECS`, `GMOUNTIE_FUSE_DEBUG`, and `GMOUNTIE_FUSE_DIRECT_MOUNT` had no Viper env-mirror registration, so setting them had no effect. Fixed; all four are now wired and honoured.
...
- **`NewClient` readahead defaults fixed (library importers).** `NewClient` was populating `PerFileConfig` with hard-coded constants that were 16× smaller than the config defaults for `ReadaheadChunkBytes` and used 1 instead of 4 for `ReadaheadWindow`. Any `pkg/client` importer that relied on `NewClient` without explicit overrides got readahead that never fires in practice (1 MiB chunk at 3-read threshold → effective prefetch for sustained sequential reads, but only if the workload happened to exceed those constants) and a window that stalled the pipeline at one in-flight chunk. Now `NewClient` reads from the same `config.Default*` constants that the CLI uses.
- **Configurable FUSE attr/entry timeout knobs; `GMOUNTIE_FUSE_*` env vars fixed.** `fuse.attr_timeout` and `fuse.entry_timeout` are now settable (env `GMOUNTIE_FUSE_ATTR_TIMEOUT_SECS` / `GMOUNTIE_FUSE_ENTRY_TIMEOUT_SECS`; config `fuse.attr_timeout` / `fuse.entry_timeout`; default 5 s). All `GMOUNTIE_FUSE_*` environment variables were silently ignored due to a missing env-mirror registration in the Viper setup — `fuse.attr_timeout`, `fuse.entry_timeout`, `fuse.debug`, and `fuse.direct_mount` were all unreachable via env vars. Fixed.
- **WAN throughput: deeper default readahead window + pinned gRPC flow-control windows.** The default `rpc.readahead_window` is raised 4 → 16: a shallow window stutters under WAN RTT (the throughput investigation measured a window of 4 reaching only ~71% of a 100 Mbit / 50 ms link vs ~82% at 16), and 16 × 1 MiB in flight per sequential reader is harmless on LAN. Two new knobs, `rpc.initial_conn_window_bytes` (default 16 MiB) and `rpc.initial_stream_window_bytes` (default 8 MiB), pin the gRPC HTTP/2 connection- and stream-level flow-control windows so a single connection can fill a high-bandwidth-delay link that gRPC's BDP autotuner under-serves (a 1 Gbit / 50 ms single stream measured ~36 vs ~52 MiB/s raw TCP; pinning lifted it +31% and left a 100 Mbit link unchanged). Set both to `0` to restore gRPC BDP autotuning; setting either one nonzero disables autotuning globally, so set the two together.
- **Readahead partial-prefix serve; tail-only live fetch.** A FUSE read whose leading bytes are covered by warm prefetch chunks no longer discards that work: the covered prefix is served from cache and only the uncovered tail is fetched live, keeping the FUSE read result full-length (a short read signals EOF to the kernel). The Observe call is issued once for the combined cache+live extent so the sequential detector advances correctly and in-flight chunks overlapping the live tail are evicted cleanly.
...
- **Mutation events cost nothing when nobody listens.** Every successful mutation used to pay a post-mutation stat (the Write path: one `openat2`+`fstatat`+`close` per RPC) to seed the cache-invalidation event's version, even with zero subscribers on the volume. The emit helpers now skip both the stat and the emit when the volume has no `Subscribe` stream attached; a subscriber attaching mid-mutation starts Unverified and revalidates, so nothing is missed.
- **Per-op credential switch no longer re-reads baseline groups.** The server's own supplementary groups are constant for the process lifetime; they're now read once and cached instead of one `getgroups` syscall plus a slice allocation on every identity-switched operation.
- **Experimental, opt-in: pre-credentialed executor for constant-identity volumes.** `squash` and `static` volumes can route filesystem ops through a small pool of OS threads whose fs credentials were switched once at startup, instead of paying a thread pin + ~9 credential syscalls around every operation. One pool exists per distinct identity (uid/gid/groups/capability mode), shared across volumes and principals. **Off by default** (`server.identity.executor_workers: 0`): micro-benchmarks put the executor's dispatch overhead (~11.5 µs) well above the per-op switch it replaces (~1.2 µs stubbed; plausibly ~6 µs with real credential syscalls), so it ships disabled pending the release perf series; enable with a positive worker count (suggested: 4). `system` and `passthrough` modes always keep per-op switching (their identities can change between requests), as does any volume where pool startup fails (e.g. insufficient privilege).
