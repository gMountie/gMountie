...
- **Constant-identity volumes skip the per-op credential switch.** `squash` and `static` volumes (the default deployment modes) now route filesystem ops through a small pool of OS threads whose fs credentials were switched once at startup, instead of paying a thread pin + ~9 credential syscalls around every operation. One pool exists per distinct identity (uid/gid/groups/capability mode), shared across volumes and principals; pool size via `server.identity.executor_workers` (default `min(4, GOMAXPROCS)`). `system` and `passthrough` modes keep per-op switching (their identities can change between requests), as does any volume where pool startup fails (e.g. insufficient privilege).
