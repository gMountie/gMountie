...
// io.FileSystemBackend. It exists so the FileSystemBackend behavioral
// contract can be exercised against a known-correct backend (see
// pkg/client/io/contract) and so future semantic layers (the WAL, a
...
// attr snapshots the node into an io.Attr. Caller holds the fs lock.
func (n *node) attr() *io.Attr {
	sz := n.size()
	return &io.Attr{
		Ino:     n.ino,
		Size:    sz,
...
func (h *memHandle) Unwrap() io.FileHandle { return h }
...
func resolveHandle(fh io.FileHandle) (*node, proto.FsError) {
	cur := fh
	for cur != nil {
...
func (fs *memFS) Stat(_ context.Context, path string) (*io.Attr, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
	n, st := fs.lookup(path)
	if st != proto.FsError_FS_OK {
...
func (fs *memFS) GetAttrIfChanged(_ context.Context, path string, knownVersion uint64) (*io.Attr, bool, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
	n, st := fs.lookup(path)
...
func (fs *memFS) Lookup(_ context.Context, parent, name string) (*io.Attr, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
...
func (fs *memFS) ListDir(_ context.Context, path string) ([]io.DirEntryPlus, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
	n, st := fs.lookup(path)
	if st != proto.FsError_FS_OK {
	...
	out := make([]io.DirEntryPlus, 0, len(n.children))
	for name, child := range n.children {
		out = append(out, io.DirEntryPlus{
			DirEntry: io.DirEntry{Ino: child.ino, Mode: child.mode, Name: name},
			Attr:     child.attr(),
		})
	}
...
func (fs *memFS) StatFs(_ context.Context, path string) (*io.StatFs, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
...
func (fs *memFS) Open(_ context.Context, path string, _ uint32) (io.FileHandle, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
	n, st := fs.lookup(path)
	if st != proto.FsError_FS_OK {
...
func (fs *memFS) Create(_ context.Context, parent, name string, flags, mode uint32) (io.FileHandle, *io.Attr, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
	pn, st := fs.lookup(parent)
	if st != proto.FsError_FS_OK {
...
func (fs *memFS) Read(_ context.Context, fh io.FileHandle, off int64, dest []byte) (int, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
...
func (fs *memFS) Write(_ context.Context, fh io.FileHandle, off int64, data []byte) (uint32, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
	n, st := resolveHandle(fh)
	if st != proto.FsError_FS_OK {
...
func (fs *memFS) Release(_ context.Context, fh io.FileHandle) proto.FsError {
	_, st := resolveHandle(fh)
	return st
}

func (fs *memFS) Flush(_ context.Context, fh io.FileHandle) proto.FsError {
	_, st := resolveHandle(fh)
	return st
}

func (fs *memFS) Fsync(_ context.Context, fh io.FileHandle, _ int64) proto.FsError {
	_, st := resolveHandle(fh)
	return st
}

func (fs *memFS) Allocate(_ context.Context, fh io.FileHandle, off, size uint64, _ uint32) proto.FsError {
	fs.mu.Lock()
	defer fs.mu.Unlock()
	n, st := resolveHandle(fh)
	if st != proto.FsError_FS_OK {
...
func (fs *memFS) CopyFileRange(_ context.Context, fhIn io.FileHandle, offIn uint64, fhOut io.FileHandle, offOut uint64, length, _ uint64) (uint64, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
...
func (fs *memFS) Lseek(_ context.Context, fh io.FileHandle, offset uint64, whence uint32) (uint64, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
	n, st := resolveHandle(fh)
	if st != proto.FsError_FS_OK {
...
func (fs *memFS) Mkdir(_ context.Context, path string, mode uint32) (*io.Attr, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
...
func (fs *memFS) Symlink(_ context.Context, target, linkPath string) (*io.Attr, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
...
func (fs *memFS) SetAttr(_ context.Context, path string, in io.SetAttrIn) (*io.Attr, proto.FsError) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
	n, st := fs.lookup(path)
	if st != proto.FsError_FS_OK {
