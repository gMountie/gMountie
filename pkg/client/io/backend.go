...
// the wire DATA types in plain io.* structs (Attr/DirEntry/StatFs) so the
// adapter and cache stack don't pass *proto messages around. Note this is only
// a partial decoupling: the interface still uses proto.FsError as its
// errno/error type throughout — the generated wire error enum doubles as the
// client-side domain errno (a full errno-type split is out of scope).
type Attr struct {
	Ino       uint64
	Size      uint64
	Blocks    uint64
