...
func (fs *LocalFileSystem) Open(name string, flags uint32, fctx *fuse.Context) (file nodefs.File, code fuse.Status) {
	...
	return NewGrpcFile(fs.client.File(), fs.volume, name, res.Fd, fs.client.IOTimeout(), fs.client.SessionID(), fs.client.ReadaheadChunkBytes(), fs.client.ReadaheadThreshold(), fs.client.WriteCoalesceBytes()), fuse.Status(res.Status)
}
