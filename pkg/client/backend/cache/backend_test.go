...
func (s *CachedBackendTestSuite) openCachedHandle(path string) (backend.FileHandle, *iomocks.MockFileHandle) {
...
func (s *CachedBackendTestSuite) TestStatHitAfterMiss() {
	s.inner.EXPECT().Stat(mock.Anything, "/x").Return(&backend.Attr{Ino: 1, Size: 10}, proto.FsError_FS_OK).Once()
	// Miss: hits inner.
...
func (s *CachedBackendTestSuite) TestLookupCachesPositiveOnSuccess() {
	s.inner.EXPECT().Lookup(mock.Anything, "/d", "child").
		Return(&backend.Attr{Ino: 7}, proto.FsError_FS_OK).Once()
...
func (s *CachedBackendTestSuite) TestListDirHitAfterMiss() {
	entries := []backend.DirEntryPlus{
		{DirEntry: backend.DirEntry{Name: "a"}},
		{DirEntry: backend.DirEntry{Name: "b"}},
	}
...
func (s *CachedBackendTestSuite) TestListDirPlusPrimesAttrCache() {
	entries := []backend.DirEntryPlus{
		{
			DirEntry: backend.DirEntry{Name: "child", Ino: 7, Mode: 0o100644},
			Attr:     &backend.Attr{Ino: 7, Size: 42, Mode: 0o100644},
		},
	}
	s.inner.EXPECT().ListDir(mock.Anything, "/d").Return(entries, proto.FsError_FS_OK).Once()
...
func (s *CachedBackendTestSuite) TestListDirNilAttrEntriesDoNotPrime() {
	entries := []backend.DirEntryPlus{{DirEntry: backend.DirEntry{Name: "noattr", Ino: 8}}}
	s.inner.EXPECT().ListDir(mock.Anything, "/d").Return(entries, proto.FsError_FS_OK).Once()
	_, st := s.b.ListDir(context.Background(), "/d")
	s.Require().Equal(proto.FsError_FS_OK, st)

	s.inner.EXPECT().Stat(mock.Anything, "/d/noattr").
		Return(&backend.Attr{Ino: 8}, proto.FsError_FS_OK).Once()
...
func (s *CachedBackendTestSuite) TestReadFetchesAndCachesOnMiss() {
	...
	s.inner.EXPECT().Read(mock.Anything, innerH, int64(0), mock.MatchedBy(func(b []byte) bool { return len(b) == 1024 })).
		RunAndReturn(func(_ context.Context, _ backend.FileHandle, _ int64, buf []byte) (int, proto.FsError) {
			for i := range buf {
				buf[i] = byte(i % 251)
			}
...
func fillReturn(_ context.Context, _ backend.FileHandle, _ int64, buf []byte) (int, proto.FsError) {
	for i := range buf {
		buf[i] = byte(i % 251)
	}
...
func (s *CachedBackendTestSuite) TestWriteUpdatesAttrAndInvalidatesData() {
	s.b.attr.putPositive("/f", &backend.Attr{Ino: 1, Size: 100})
...
func (s *CachedBackendTestSuite) TestCreateBumpsParentDirAndDropsNegative() {
	...
	s.b.attr.putPositive("/d", &backend.Attr{Ino: 1, Mtime: 1000})
	s.b.dir.put("/d", []backend.DirEntry{{Name: "x"}})
	innerH := iomocks.NewMockFileHandle(s.T())
	innerH.EXPECT().Unwrap().Return(innerH).Maybe()
...
func (s *CachedBackendTestSuite) TestCreatePopulatesPositiveAttrIfBackendReturns() {
	innerH := iomocks.NewMockFileHandle(s.T())
	innerH.EXPECT().Unwrap().Return(innerH).Maybe()
	returnedAttr := &backend.Attr{Ino: 42, Size: 0, Mode: 0o100644}
	s.inner.EXPECT().Create(mock.Anything, "/d", "new", mock.Anything, mock.Anything).
...
func (s *CachedBackendTestSuite) TestMkdirInvalidatesParentAndDropsNegative() {
	...
	s.b.attr.putPositive("", &backend.Attr{Ino: 1}) // parent attr cached
	s.b.dir.put("", []backend.DirEntry{})
...
func (s *CachedBackendTestSuite) TestMkdirPrimesAttrCacheFromReply() {
	returned := &backend.Attr{Ino: 21, Mode: 0o40755}
...
func (s *CachedBackendTestSuite) TestSymlinkBumpsParentAndPrimes() {
	s.b.attr.putPositive("/d", &backend.Attr{Ino: 1, Mtime: 1000}) // parent attr cached
	s.b.dir.put("/d", []backend.DirEntry{})
	returned := &backend.Attr{Ino: 31, Mode: 0o120777}
...
func (s *CachedBackendTestSuite) TestRmdirInvalidatesAndNegativesPath() {
	s.b.attr.putPositive("/d", &backend.Attr{Ino: 1})
	s.b.dir.put("/d", []backend.DirEntry{})
	s.b.dir.put("", []backend.DirEntry{{Name: "d"}})
...
func (s *CachedBackendTestSuite) TestUnlinkInvalidatesAndNegativesPath() {
	s.b.attr.putPositive("/f", &backend.Attr{Ino: 1})
	s.b.data.put("/f", 0, []byte("c"))
	s.b.dir.put("", []backend.DirEntry{{Name: "f"}})
...
func (s *CachedBackendTestSuite) TestRenameInvalidatesBothPaths() {
	s.b.attr.putPositive("/a", &backend.Attr{Ino: 1})
	s.b.attr.putPositive("/b", &backend.Attr{Ino: 2})
...
func (s *CachedBackendTestSuite) TestRenameInvalidatesOldParentAttr() {
	...
	s.b.attr.putPositive("src", &backend.Attr{Ino: 10})
...
func (s *CachedBackendTestSuite) TestRenameInvalidatesNewParentAttr() {
	...
	s.b.attr.putPositive("dst", &backend.Attr{Ino: 20})
...
func (s *CachedBackendTestSuite) TestSetAttrWithSizeInvalidatesDataAndPrimesAttr() {
	s.b.data.put("/f", 0, make([]byte, 1024))
	s.b.data.put("/f", 1, make([]byte, 1024))
	s.b.attr.putPositive("/f", &backend.Attr{Size: 2048, Mode: 0o644})
	in := backend.SetAttrIn{Valid: backend.FATTR_SIZE | backend.FATTR_MODE, Size: 100, Mode: 0o600}
	final := &backend.Attr{Ino: 9, Size: 100, Mode: 0o600}
...
func (s *CachedBackendTestSuite) TestSetAttrWithoutSizeKeepsDataAndPrimesAttr() {
	...
	s.b.attr.putPositive("/f", &backend.Attr{Mode: 0o644})
	in := backend.SetAttrIn{Valid: backend.FATTR_MODE, Mode: 0o600}
	s.inner.EXPECT().SetAttr(mock.Anything, "/f", in).
		Return(&backend.Attr{Mode: 0o600}, proto.FsError_FS_OK).Once()
...
func (s *CachedBackendTestSuite) TestSetAttrFailureStillInvalidates() {
	s.b.data.put("/f", 0, []byte("DATA"))
	s.b.attr.putPositive("/f", &backend.Attr{Size: 4, Mode: 0o644})
	in := backend.SetAttrIn{Valid: backend.FATTR_SIZE | backend.FATTR_MODE, Size: 0, Mode: 0o600}
...
func (s *CachedBackendTestSuite) TestAllocateInvalidatesDataRangeAndKeepsAttr() {
	...
	s.b.data.put("/f", 1, make([]byte, 1024))
	s.b.data.put("/f", 2, make([]byte, 1024))
	s.b.attr.putPositive("/f", &backend.Attr{Size: 3072})
	h, innerH := s.openCachedHandle("/f")
...
func (s *CachedBackendTestSuite) TestStatFsCachedWithinTTL() {
	...
	s.inner.EXPECT().StatFs(mock.Anything, "/").
		Return(&backend.StatFs{Bfree: 42}, proto.FsError_FS_OK).Once()
...
func (s *CachedBackendTestSuite) TestAccessInvalidatedOnSetAttr() {
	...
	s.inner.EXPECT().SetAttr(mock.Anything, "/f", mock.Anything).
		Return(&backend.Attr{}, proto.FsError_FS_OK).Once()
	s.Equal(proto.FsError_FS_OK, s.b.Access(context.Background(), "/f", 4))
	_, _ = s.b.SetAttr(context.Background(), "/f", backend.SetAttrIn{})
	s.Equal(proto.FsError_FS_OK, s.b.Access(context.Background(), "/f", 4))
}
...
func (s *CachedBackendTestSuite) TestUnverifiedStateGatesStatViaGetAttrIfChanged() {
	...
	inner.EXPECT().Stat(mock.Anything, "f").Return(&backend.Attr{Ino: 7, Size: 11, Version: 42}, proto.FsError_FS_OK).Once()
...
func (s *CachedBackendTestSuite) TestUnverifiedRevalidationOnVersionChangeInvalidatesAllThree() {
	...
	// Seed the cache.
	inner.EXPECT().Stat(mock.Anything, "f").Return(&backend.Attr{Version: 42}, proto.FsError_FS_OK).Once()
	_, _ = be.Stat(context.Background(), "f")

	// Version changed: server returns new attrs.
	freshAttr := &backend.Attr{Version: 99, Size: 200}
...
func (s *CachedBackendTestSuite) TestCopyFileRangeInvalidatesDestination() {
	...
	// Seed dst attr cache.
	s.inner.EXPECT().Stat(mock.Anything, "/dst").Return(&backend.Attr{Ino: 2, Size: 4096}, proto.FsError_FS_OK).Once()
	...
	s.inner.EXPECT().Read(mock.Anything, innerDst, int64(0), mock.Anything).
		RunAndReturn(func(_ context.Context, _ backend.FileHandle, _ int64, b []byte) (int, proto.FsError) {
			return 1024, proto.FsError_FS_OK
		}).Once()
...
//
// Lseek now forwards via the embedded backend.PassthroughBackend, which passes the
...
func (s *CachedBackendTestSuite) TestListDirPrimesXattrCache() {
	s.inner.EXPECT().ListDir(mock.Anything, "d").Return([]backend.DirEntryPlus{{
		DirEntry:    backend.DirEntry{Name: "child"},
		XattrListed: true,
...
func (s *CachedBackendTestSuite) TestRenameInvalidatesDescendantsAcrossDiskTier() {
	...
	be.attr.putPositive("d/.git", &backend.Attr{Ino: 7})
...
func (s *CachedBackendTestSuite) TestCreatePrimesSecurityCapabilityNegative() {
	...
	inner.EXPECT().Create(mock.Anything, "/d", "f", mock.Anything, mock.Anything).
		Return(innerH, &backend.Attr{Ino: 9}, proto.FsError_FS_OK).Once()
...
func (s *CachedBackendTestSuite) TestCreateThenStatParentHitsCache() {
	s.b.attr.putPositive("/d", &backend.Attr{Ino: 1, Mode: 0o40755, Mtime: 1000})
	innerH := iomocks.NewMockFileHandle(s.T())
	innerH.EXPECT().Unwrap().Return(innerH).Maybe()
...
func (s *CachedBackendTestSuite) TestAttrMissRecordsViaInjectedRecorder() {
	...
	inner := iomocks.NewMockFileSystemBackend(s.T())
	inner.EXPECT().Stat(mock.Anything, "/x").Return(&backend.Attr{Ino: 7, Size: 3}, proto.FsError_FS_OK).Once()
