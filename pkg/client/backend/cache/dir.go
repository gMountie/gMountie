...
// view.
func (c *dirCache) get(path string) ([]backend.DirEntry, bool) {
	e := c.st.get(path)
	if e == nil {
		return nil, false
	}
	...
	out := make([]backend.DirEntry, len(de.entries))
	copy(out, de.entries)
	return out, true
}

func (c *dirCache) put(path string, entries []backend.DirEntry) {
	stored := make([]backend.DirEntry, len(entries))
	copy(stored, entries)
...
// backend.DirEntry (struct fields + the name string header).
