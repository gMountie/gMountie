...
func attrFromProto(p *proto.Attr) *backend.Attr {
	if p == nil {
		return nil
	}
	out := &backend.Attr{
...
//
// In-band failures still arrive as proto.FsError in the reply body; this
// helper only covers the error arm of each RPC.
...
// Release/Allocate/locks/CopyFileRange/Lseek) to a proto.FsError. It differs from
...
func (b *BackendClient) Stat(ctx context.Context, path string) (*backend.Attr, proto.FsError) {
...
func (b *BackendClient) GetAttrIfChanged(ctx context.Context, path string, knownVersion uint64) (*backend.Attr, bool, proto.FsError) {
	...
	return attrFromProto(reply.GetAttrs()), false, proto.FsError_FS_OK
}

// Close is a no-op for BackendClient: the gRPC connection lifetime is
// managed by the caller (grpc.Client). Implements backend.FileSystemBackend.
...
func (b *BackendClient) Lookup(ctx context.Context, parent, name string) (*backend.Attr, proto.FsError) {
...
func (b *BackendClient) ListDir(ctx context.Context, path string) ([]backend.DirEntryPlus, proto.FsError) {
	res, err := retryOp(b.client, ctx, "ReadDir", classIdempotentRead, b.client.MetaTimeout(),
		func(ctx context.Context) (listDirResult, error) {
			...
			out := listDirResult{entries: []backend.DirEntryPlus{}, status: proto.FsError_FS_OK}
			for {
				...
				if st := batch.GetStatus(); st != proto.FsError_FS_OK {
				...
				for _, e := range batch.GetEntries() {
					...
					out.entries = append(out.entries, backend.DirEntryPlus{
	...
	return res.entries, proto.FsError_FS_OK
}

// Access mirrors access(2). Idempotent.
func (b *BackendClient) Access(ctx context.Context, path string, mode uint32) proto.FsError {
...
func (b *BackendClient) Readlink(ctx context.Context, path string) (string, proto.FsError) {
...
// gRPC error, otherwise the reply plus the proto.FsError from statusOf (the
...
func mutatePath[Rep any](
	...
	statusOf func(Rep) proto.FsError,
	logFields ...zap.Field,
) (Rep, proto.FsError) {
...
func (b *BackendClient) Symlink(ctx context.Context, target, linkPath string) (*backend.Attr, proto.FsError) {
...
func (b *BackendClient) StatFs(ctx context.Context, path string) (*backend.StatFs, proto.FsError) {
...
func (b *BackendClient) GetXAttr(ctx context.Context, path, attr string) ([]byte, proto.FsError) {
...
func (b *BackendClient) ListXAttr(ctx context.Context, path string) ([]string, proto.FsError) {
	...
	return res.Attributes, res.Status
}
...
// back to Stat).
func (b *BackendClient) Mkdir(ctx context.Context, path string, mode uint32) (*backend.Attr, proto.FsError) {
...
const setAttrValidMask = backend.FATTR_MODE | backend.FATTR_UID | backend.FATTR_GID |
	backend.FATTR_SIZE | backend.FATTR_ATIME | backend.FATTR_MTIME
...
func (b *BackendClient) SetAttr(ctx context.Context, path string, in backend.SetAttrIn) (*backend.Attr, proto.FsError) {
...
func (b *BackendClient) Open(ctx context.Context, path string, flags uint32) (backend.FileHandle, proto.FsError) {
...
func (b *BackendClient) Create(ctx context.Context, parent, name string, flags, mode uint32) (backend.FileHandle, *backend.Attr, proto.FsError) {
...
//     pre-readahead path.
func (b *BackendClient) Read(ctx context.Context, fh backend.FileHandle, off int64, dest []byte) (int, proto.FsError) {
	h := resolveHandle(fh)
	if h == nil {
	...
	if h.readahead != nil {
		...
		if n > 0 {
			...
			return n + m, proto.FsError_FS_OK
		}
	}
...
// differences are the FileHandle boundary and the (int, proto.FsError)
// return shape. A clean stream end before dest is full returns the short
// count with proto.FsError_FS_OK (EOF); a non-OK status returns (0, status).
//
// Idempotent: each retry attempt opens a fresh stream; no request_id.
func (b *BackendClient) readLive(ctx context.Context, h *grpcFileHandle, off int64, dest []byte) (int, proto.FsError) {
	...
	res, err := retryOp(h.client, ctx, "Read", classFdOp, h.ioTimeout, func(ctx context.Context) (readResult, error) {
		if st := h.reclaimIfStale(ctx); st != proto.FsError_FS_OK {
			return readResult{}, errFromStatus(st)
		}
		snap := h.state.Load()
		dataClient, releaseConn := h.client.DataFileClient()
		defer releaseConn()
		stream, err := dataClient.Read(ctx, &proto.ReadRequest{
			Volume:    h.volume,
			Fd:        snap.fd,
			Offset:    off,
		...
		out := readResult{status: proto.FsError_FS_OK}
		for {
			...
			if st := frame.GetStatus(); st != proto.FsError_FS_OK {
				out.status = st
				return out, nil
			}
			data := frame.GetData()
	...
	return res.written, proto.FsError_FS_OK
}
...
func (b *BackendClient) streamingWrite(h *grpcFileHandle, data []byte, off int64, requestID string) (uint32, proto.FsError) {
	...
	res, err := retryOp(h.client, context.Background(), "Write", classFdOp, h.ioTimeout,
		func(ctx context.Context) (*proto.WriteReply, error) {
			...
			dataClient, releaseConn := h.client.DataFileClient()
			defer releaseConn()
			stream, err := dataClient.Write(ctx, grpc.WaitForReady(true))
			if err != nil {
				return nil, err
			}
...
func (b *BackendClient) Fsync(ctx context.Context, fh backend.FileHandle, flags int64) proto.FsError {
	h := resolveHandle(fh)
	if h == nil {
		return proto.FsError_FS_EBADF
	}
...
func (b *BackendClient) Allocate(ctx context.Context, fh backend.FileHandle, off, size uint64, mode uint32) proto.FsError {
	h := resolveHandle(fh)
	if h == nil {
		return proto.FsError_FS_EBADF
	}
...
func (b *BackendClient) CopyFileRange(ctx context.Context, fhIn backend.FileHandle, offIn uint64, fhOut backend.FileHandle, offOut uint64, length, flags uint64) (uint64, proto.FsError) {
	...
	res, err := retryOp(dst.client, ctx, "CopyFileRange", classFdOp, dst.ioTimeout,
		func(ctx context.Context) (*proto.CopyFileRangeReply, error) {
			if st := src.reclaimIfStale(ctx); st != proto.FsError_FS_OK {
				return nil, errFromStatus(st)
			}
			if st := dst.reclaimIfStale(ctx); st != proto.FsError_FS_OK {
				return nil, errFromStatus(st)
			}
...
func (b *BackendClient) Lseek(ctx context.Context, fh backend.FileHandle, offset uint64, whence uint32) (uint64, proto.FsError) {
	h := resolveHandle(fh)
	if h == nil {
		return 0, proto.FsError_FS_EBADF
	}
...
func (b *BackendClient) GetLk(ctx context.Context, fh backend.FileHandle, owner uint64, lk *backend.FileLock, flags uint32, out *backend.FileLock) proto.FsError {
	h := resolveHandle(fh)
	if h == nil {
		return proto.FsError_FS_EBADF
	}
...
func (b *BackendClient) SetLk(ctx context.Context, fh backend.FileHandle, owner uint64, lk *backend.FileLock, flags uint32) proto.FsError {
	h := resolveHandle(fh)
	if h == nil {
		return proto.FsError_FS_EBADF
	}
...
func (b *BackendClient) SetLkw(ctx context.Context, fh backend.FileHandle, owner uint64, lk *backend.FileLock, flags uint32) proto.FsError {
	h := resolveHandle(fh)
	if h == nil {
		return proto.FsError_FS_EBADF
	}
	...
	defer cancel()
	if reclaimSt := h.reclaimIfStale(ctx2); reclaimSt != proto.FsError_FS_OK {
		return reclaimSt
	}
	snap := h.state.Load()
...
func (h *grpcFileHandle) Unwrap() backend.FileHandle { return h }
...
func resolveHandle(fh backend.FileHandle) *grpcFileHandle {
	cur := fh
	for cur != nil {
