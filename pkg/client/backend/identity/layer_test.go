...
// by pkg/client/backend/node_idrewrite_test.go into the layer's own home.
...
func (s *LayerSuite) setOwnership(path string, uid, gid uint32) {
	_, st := s.inner.SetAttr(s.ctx, path, backend.SetAttrIn{
		Valid: backend.FATTR_UID | backend.FATTR_GID, Uid: uid, Gid: gid,
	})
...
func (s *LayerSuite) TestSetAttrOutboundRewriteBothBits() {
	...
	a, st := s.layer.SetAttr(s.ctx, "chown", backend.SetAttrIn{
		Valid: backend.FATTR_UID | backend.FATTR_GID, Uid: localUID, Gid: localGID,
	})
	s.Require().Equal(proto.FsError_FS_OK, st)
...
func (s *LayerSuite) TestSetAttrOutboundUIDOnlyStillRewrites() {
	...
	a, st := s.layer.SetAttr(s.ctx, "chuid", backend.SetAttrIn{
		Valid: backend.FATTR_UID, Uid: localUID, // only uid bit set
	})
	s.Require().Equal(proto.FsError_FS_OK, st)
...
func (s *LayerSuite) TestSetAttrNonOwnerBitsNoRewrite() {
	...
	_, st := s.layer.SetAttr(s.ctx, "sz", backend.SetAttrIn{Valid: backend.FATTR_SIZE, Size: 7})
	s.Require().Equal(proto.FsError_FS_OK, st)
...
func (a *aliasingInner) Stat(context.Context, string) (*backend.Attr, proto.FsError) {
	return a.attr, proto.FsError_FS_OK
}

func (a *aliasingInner) Lookup(context.Context, string, string) (*backend.Attr, proto.FsError) {
	return a.attr, proto.FsError_FS_OK
}

func (a *aliasingInner) ListDir(context.Context, string) ([]backend.DirEntryPlus, proto.FsError) {
	return []backend.DirEntryPlus{{
		DirEntry: backend.DirEntry{Name: "child", Ino: 1},
		Attr:     a.entry,
	}}, proto.FsError_FS_OK
}
...
func (s *LayerSuite) TestInboundDoesNotMutateInnerAttr() {
	inner := &aliasingInner{
		attr:  &backend.Attr{Ino: 1, Uid: serverUID, Gid: serverGID},
		entry: &backend.Attr{Ino: 2, Uid: serverUID, Gid: serverGID},
	}
...
func (s *LayerSuite) TestNilRewriterNoInboundRewrite() {
	...
	_, st = inner.SetAttr(s.ctx, "raw", backend.SetAttrIn{
		Valid: backend.FATTR_UID | backend.FATTR_GID, Uid: foreignID, Gid: foreignID,
	})
	s.Require().Equal(proto.FsError_FS_OK, st)
