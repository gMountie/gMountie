...
// conformance suite for backend.FileSystemBackend. Any layer (the in-memory
...
func RunBackendContract(t *testing.T, name string, newBackend func() backend.FileSystemBackend) {
	t.Helper()
	b := newBackend()
	t.Cleanup(func() {
	...
	t.Run(name+"/SetAttrSizeTruncation", func(t *testing.T) {
		...
		_, st = b.SetAttr(ctx, "trunc.txt", backend.SetAttrIn{Valid: backend.FATTR_SIZE, Size: 4})
		require.Equal(t, proto.FsError_FS_OK, st)
