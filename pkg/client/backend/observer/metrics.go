...
func NewMetricsLayer(inner backend.FileSystemBackend, rec metrics.Recorder) backend.FileSystemBackend {
	return &metricsLayer{PassthroughBackend: backend.PassthroughBackend{Inner: inner}, rec: rec}
}

func (l *metricsLayer) Stat(ctx context.Context, path string) (*backend.Attr, proto.FsError) {
	start := time.Now()
...
func (l *metricsLayer) Read(ctx context.Context, fh backend.FileHandle, off int64, dest []byte) (int, proto.FsError) {
	start := time.Now()
...
func (l *metricsLayer) Write(ctx context.Context, fh backend.FileHandle, off int64, data []byte) (uint32, proto.FsError) {
	start := time.Now()
...
func (l *metricsLayer) Lookup(ctx context.Context, parent, name string) (*backend.Attr, proto.FsError) {
	start := time.Now()
...
func (l *metricsLayer) ListDir(ctx context.Context, path string) ([]backend.DirEntryPlus, proto.FsError) {
	start := time.Now()
